		...
		if(mysql_num_rows($query) == 0){	//ini artinya jika data hasil query di atas kosong
			...
			while($data = mysql_fetch_assoc($query)){	//perulangan while dg membuat variabel $data yang akan mengambil data di database
				...
				echo '<tr style="height: 40px; text-align: center;">';
					...
					echo '<td>'.$data['tahun'].'</td>';		
					echo '<td style="background-color:#ccff66"><a href="tambah_swot.php?kode_res='.$data['kode_res'].'" target="_blank">Tambah</a></td>';
					echo '<td style="background-color:#66ff99"><a href="hasil-swot.php?kode_res='.$data['kode_res'].'" target="_blank">Lihat</a></td>';
